...
impl StateRepository<State> for AgentStateRepository {
    ...
    async fn load(&self) -> Result<State, StateError> {
        // Ok(self.read_file(&self.state_repo_path).await?)
    ...
    async fn store(&self, state: &State) -> Result<(), StateError> {
        ...
        if !self.state_repo_path.exists() {
            let () = fs::create_dir(&self.state_repo_path).await?;
        }
...
impl AgentStateRepository {
    pub fn new(config_location: &TEdgeConfigLocation) -> Self {
        let mut state_repo_path = config_location.tedge_config_root_path.clone();
        state_repo_path.push(PathBuf::from_str(".agent/software").expect("infallible"));
